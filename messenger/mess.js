...
if (!token || !currentUserId) {
  ...
  window.location.href =
    "http://127.0.0.1:5500/front-end/facebook-clone-fe/sign_in.html";
}
    ...
    if (curentFriendId != null) {
      try {
        const chatRes = await fetch(
          `http://localhost:8080/api/chats/${curentFriendId}`,
          {
            headers: {
...
async function searchFriends(keyword) {
  try {
    const response = await fetch(
      `http://localhost:8080/api/friend/search/${encodeURIComponent(keyword)}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
async function loadChat(friendId, friendName) {
  ...
  try {
    ...
    currentRoomId = roomId.roomId;
    curentFriendId = roomId.roomId;
    joinRoom(roomId);

    const chatRes = await fetch(
      `http://localhost:8080/api/chats/${currentRoomId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
